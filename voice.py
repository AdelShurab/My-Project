...
def speak(text):
    tts = gTTS(text=text, lang='ar')  # Use 'lang' parameter to specify the language.
    filename = "../../Desktop/ChatBot Project/freelance/freelance/temp.mp3"
    tts.save(filename)
...
def listen():
    ...
    with sr.Microphone() as source:
        ...
        try:
            return r.recognize_google(audio, language='ar-AR').lower()
        except sr.UnknownValueError:
            return None


def select_resources():
    # Ask for the preferred method (voice or text)
    method = input("Would you like to continue with voice or text (voice/text)? ").strip().lower()
    if method == "voice":
        speak("الرجاء ادخال الجنس (ذكر/انثى):")
        gender = input().strip().lower()
        speak("الرجاء ادخال الحالة الاجتماعية (أعزب/متزوج):")
        marital_state = input().strip().lower()
    else:
